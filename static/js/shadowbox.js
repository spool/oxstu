...
(function(){
    ...
    var options = {
         ...
         */
        loadingImage:       '/images/backend/loading.gif',
         ...
         */
        overlayBgImage:     'images/backend/overlay-85.png',
    ...
    var RE = {
        ...
        rel:            /^(lightbox|pagelet|shadowbox)/i, // rel attribute format
        gallery:        /^(lightbox|pagelet|shadowbox)\[(.*?)\]/i, // rel attribute format for gallery link
    ...
    Shadowbox.init = function(opts){
        ...
        RE.iframe = new RegExp('(/\admin/\pl/\(\w+/\)*)|(\.(' + options.ext.iframe.join('|') + '))\s*$', 'i');
    ...
    Shadowbox.setup = function(links, opts){
        ...
        for(var i = 0, len = links.length; i < len; ++i){
            ...
            if(typeof link.shadowboxCacheKey == 'undefined'){
                // find pagelet items and add height, width and "im a clever client" to GET string
                // --josh
                rel = link.getAttribute('rel');
                if (rel) {
                match = rel.match(RE.rel)
                if (match) {
                if (match[0].match(/^pagelet$/i))
                {
                    // check what parameters are there
                    ...
                    for(var i = 0, len = params.length; i < len; ++i){
                        ...
                        if(match){
                            if(match[1].match(/^height$/i)){
                                h = match[2];
                            } else if (match[1].match(/^width$/i)){
                                w = match[2];
                            }
                        }
                    }
                    // append the string to the url in href
                    href = link.getAttribute('href')
                    if (href)
                    {
                        if (href.indexOf('?')>0)
                        {
                            link.setAttribute('href', href + '&width=' + w + '&height=' + h + '&cleverclient=1');
                        } else {
                            link.setAttribute('href', href + '?width=' + w + '&height=' + h + '&cleverclient=1');
                        }
                    }
